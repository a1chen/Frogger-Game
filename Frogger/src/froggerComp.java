...
public class froggerComp extends GameDriverV3 implements KeyListener {
	...
	frog mainFrog;
	Lane[] lanes;
	BufferedImage frog, smallFrog, frogSpriteRight, frogSpriteLeft, frogSpriteUp, frogSpriteDown, wood, woodSmall, woodMedium, woodLarge, cars, carSmallDown, 
	carSmallUp, truckUp, truckDown;
	...
	public froggerComp() {
		...
		frogSpriteRight = smallFrog.getSubimage(smallFrog.getWidth()/5*0, smallFrog.getHeight()/8*2, smallFrog.getWidth()/5, smallFrog.getHeight()/8);
		frogSpriteLeft = smallFrog.getSubimage(smallFrog.getWidth()/5*0, smallFrog.getHeight()/8*6, smallFrog.getWidth()/5, smallFrog.getHeight()/8);
		frogSpriteUp = smallFrog.getSubimage(smallFrog.getWidth()/5*0, smallFrog.getHeight()/8*0, smallFrog.getWidth()/5, smallFrog.getHeight()/8);
		frogSpriteDown = smallFrog.getSubimage(smallFrog.getWidth()/5*0, smallFrog.getHeight()/8*4, smallFrog.getWidth()/5, smallFrog.getHeight()/8);
		...
		carSmallDown = cars.getSubimage(cars.getWidth()/5 * 0, cars.getHeight()/8 * 0, cars.getWidth()/5-30, cars.getHeight()/8);
		carSmallUp = cars.getSubimage(cars.getWidth()/5 * 4 +30, cars.getHeight()/8 * 1, cars.getWidth()/5-30, cars.getHeight()/8);
	...
	public void start() {
		...
		for (int i = 0; i < 16; i++) { // initialize different lanes
			lanes[i] = new Lane(100 * (i + 1), randomizer.nextInt(3), mainFrog); // randomizer decides what type of lane
			this.addKeyListener(lanes[i]);
		}
	...
	public void draw(Graphics2D win) {
		...
		if (gameState == 1) {
			...
			for(int i = 0; i<16; i++) {
				if(lanes[i].decide == 1) {
					win.drawImage(woodSmall, null, (int)lanes[i].main.logMain.getX(), (int)lanes[i].main.logMain.getY()); //draws wood
					win.drawImage(woodMedium, null, (int)lanes[i].main.log2.getX(), (int)lanes[i].main.log2.getY());
			...
			win.drawLine(0, 700, 1830, 700);
			if(mainFrog.right) {
				win.drawImage(frogSpriteRight, null, (int)mainFrog.getX(), (int)mainFrog.getY()-25); //drawing frog right
			}
			if(mainFrog.left) {
	...
	public void keyPressed(KeyEvent e) {
		...
		if (mainFrog.getX() > 1600 && !mainFrog.madeAcross && e.getKeyCode() == KeyEvent.VK_RIGHT) { // changes map
			...
			for (int i = 0; i < 16; i++) {
				lanes[i].decide = randomizer.nextInt(3);
			}
